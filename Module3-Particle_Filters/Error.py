...
def eval(r, p):
    ...
    for i in range(len(p)):  # calculate mean error
        dx = (p[i].x - r.x + (world_size / 2.0)) % world_size - (world_size / 2.0)  # Note: The world is cyclic.
        dy = (p[i].y - r.y + (world_size / 2.0)) % world_size - (world_size / 2.0)  # Note: The world is cyclic.
